...
const addToQueueAndWait = function(callWhenReady) {
  return new Promise((resolve, reject) => {
    callQueue.enqueue({ callWhenReady, resolve, reject });
    execQueue();
  });
};
...
const throttleAndQueue = function(
  ...
  let promisifiedFtt = returnsPromise
    ? functionToThrottle
    : promisify(functionToThrottle);

  const throttledFunction = function(...args) {
    const callFttWhenFinishedWaiting = () => {
    ...
    return addToQueueAndWait(callFttWhenFinishedWaiting);
  };
